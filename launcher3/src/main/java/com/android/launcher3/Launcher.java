...
public class Launcher extends Activity
    ...
    //mPaused 判读activity是否在前台
    private boolean mPaused = true;

    //mRestoring 判断是否有数据恢复
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //初始化LauncherAppState单例
        LauncherAppState.setApplicationContext(getApplicationContext());
        LauncherAppState app = LauncherAppState.getInstance();

        // 初始化一个横屏的mDeviceProfile还是竖屏的mDeviceProfile
        mDeviceProfile =
                getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE ?
                        app.getInvariantDeviceProfile().landscapeProfile
                            : app.getInvariantDeviceProfile().portraitProfile;

        //这个ShardPreferences作用是：控制指引界面的显示，首次启动launcher才会出现指引界面
        //之后的启动就不会有指引界面出现
        ...
        //LauncherModel的对象, 负责数据层操作
        // LauncherAppState.setLauncher获得
        mModel = app.setLauncher(this);

        //缓存桌面图标对象, IconCache用来提高性能
        ...
        // 而这些都是通过mInflater.inflate(..)方法从xml生成出来的，
        // 使用次数还是挺多的，详情参加Launcher.creatShortcut方法。
        ...
        //mAppWidgetHost用来生成小工具的View对象并更新。
        ...
        if (!mRestoring) {
            if (DISABLE_SYNCHRONOUS_BINDING_CURRENT_PAGE) {
                //如果用户是离开launcher，我们就应该异步加载这个项
                mModel.startLoader(PagedView.INVALID_RESTORE_PAGE);
            }
            //只跑else， DISABLE_SYNCHRONOUS_BINDING_CURRENT_PAGE 为false
            else {
                // We only load the page synchronously if the user rotates (or triggers a
                // configuration change) while launcher is in the foreground
                /// launcher在前台, 我们只加载页面同步如果用户旋转(或触发配置更改)
    ...
    public void onDestroy() {
        ...
        try {
            ...
            Log.w(TAG, "problem while stopping AppWidgetHost during Launcher destruction. exception: " + ex);
        }
        mAppWidgetHost = null;
    ...
    public boolean onLongClick(View v) {
        if (!isDraggingEnabled()) return false;  // 如果不允许拖拽则返回
        if (isWorkspaceLocked()) return false;  // 如果桌面锁定返回
        if (mState != State.WORKSPACE) return false; // 如果没有在桌面显示状态返回
        ...
        //如果长按的是桌面的空白区域，调用showOverviewMode(true)来显示桌面预览状态，
        //这时再长按单个CellLayout可以进行拖拽排序。
        /*if (v instanceof Workspace) {
            if (!mWorkspace.isInOverviewMode()) {
        ...
        /**再往下，
        判断如果没有拖拽事件执行就开始判断执行拖拽事件，下面的判断也是如果CellLayout.CellInfo这个对象为空，
        则执行桌面预览或者排序事件，如果不为空，则说明长按的是图标，
        那么此时要判断你拖动的不是文件夹或者不是显示所有图标的那个按钮，开始执行*/
        CellLayout.CellInfo longClickCellInfo = null;
        View itemUnderLongClick = null;
        //长按的是app的图标或者文件夹
        if (v.getTag() instanceof ItemInfo) {
            ItemInfo info = (ItemInfo) v.getTag();
            //创建CellLayout.CellInfo对象，是对你要拖拽的View对象的信息复制一份出来
        ...
        if (!mDragController.isDragging()) {
            if (itemUnderLongClick == null) { //用户长按的是空白处
                /*mWorkspace.performHapticFeedback(HapticFeedbackConstants.LONG_PRESS,
                        HapticFeedbackConstants.FLAG_IGNORE_VIEW_SETTING);
                if (mWorkspace.isInOverviewMode()) {
                ...
                final boolean isAllAppsButton = inHotseat
                        && isAllAppsButtonRank(mHotseat.getOrderInHotseat(longClickCellInfo.cellX, longClickCellInfo.cellY));
